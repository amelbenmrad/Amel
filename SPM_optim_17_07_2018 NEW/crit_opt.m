...
%     C1_m=sum(C1(3,:).*dr.^3)./sum(dr.^3);
%     C2_m=sum(C1(3,:).*dr.^3)./sum(dr.^3);
%     T_m=sum(T(3,:).*dr.^3)./sum(dr.^3);
%       m1=C1_m*P.Mw1*dr.^3*4/3*pi;
%       m2=C2_m*P.Mw2*dr.^3*4/3*pi;
%       rho_1 = P.P * P.Mw1 / (P.R * T_m); 
%       rho_2 = P.P * P.Mw2 / (P.R * T_m); 
%       V1_part= m1*rho_1;
%       V2_part= m2*rho_2;    
%     V_part=4/3*pi*dr.^3+V1_part+V2_part;
%     rho_PE=950;
%     P.rho_ov = (m1 +m2+V_part*rho_PE)/V_part;
